...
- [ ] 导出中显示取消按钮
- [ ] 点击后调用 `cancel_gif_export`
- [ ] 进入 `cancelling` 时禁用重复提交
- [ ] 轮询到 `cancelled` 后显示明确结果

验收：取消后界面不会显示成功，不产生最终输出文件。

### A5：取消竞态和发布事务

- [ ] 增加 `publishing`/`completed` 边界
- [ ] 发布前取消必须回滚
- [ ] 发布后不再报告 cancelled
- [ ] 覆盖模式失败时恢复旧文件和 `bak`
...
- [ ] 显示当前文件名、`n/total`、成功、失败、跳过
- [ ] 增加真实 `progressbar` 和 ARIA 属性
- [ ] 导出中始终提供取消按钮
