...
- [ ] 覆盖 320×480、360×500、480×640、700×1100、1280×800
- [ ] 每个尺寸分别验证图片模式和视频模式
- [ ] 检查卡片/控件重叠、文字溢出、横向滚动条和关键按钮可达
- [ ] 保存失败截图、复现步骤和平台/运行环境

验收：五种尺寸、两种模式全部通过；无重叠、文字溢出或横向滚动条，Tab 可到达导出、取消和重试等关键按钮。

#### E2：GIF 视频抽帧资源与临时文件生命周期（依赖：C3、D2）

- [ ] 明确内存、磁盘、帧数和累计像素预算
- [ ] 覆盖正常完成、失败、取消、进程崩溃后的临时目录清理
- [ ] 增加长视频和接近预算上限的回归样本
- [ ] 记录残留扫描结果和清理耗时
...
- [ ] 覆盖大图、200 帧、长视频和混合尺寸输入
- [ ] 记录峰值内存、磁盘峰值、耗时、输出体积和失败率
- [ ] 固定工具链与样本，生成可比较的基准记录

验收：基准可重复运行，结果包含完整输入参数和环境信息；相对基线的回归阈值明确，超阈值时 CI 或验收报告失败。
