...
- [ ] 尚未单独制造导出失败并验证“仅重试失败项/重试”按钮；取消按钮已在真实抽帧中验证，但未对每个 viewport 重复记录动态取消状态

验收：五种尺寸、两种模式的真实 viewport 几何/横向溢出检查通过；480×640 与 700×1100 的真实视频源布局通过；导出按钮五尺寸 Tab 可达，长中文文件名无页面级溢出。失败重试状态尚未稳定制造，故本条暂不勾选。
