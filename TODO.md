...
- [ ] 记录三档同一输入的体积、耗时、峰值内存和质量指标

验收：三档参数在 UI 中可选择、可解释、可复现，且手动覆盖规则明确；同一输入三档的真实输出指标仍需在桌面端基准 harness 中补测后才能勾选完整验收。现有 C4/E3 的质量代理定义为首帧 RGB MAE（越低越好），并记录输出体积、编码耗时和进程峰值内存。
