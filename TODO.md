...
目标：从 `0.1.9` 稳定推进到当前版本，并持续保持可验证发布。
...
- [x] `metadataPolicy=preserve` 返回源图实际位深，并补充透明 PNG 回归测试
- [x] 补充导出格式回读、透明度、尺寸和临时文件清理测试
- [x] 更新下载支持带 ETag 的 Range 断点续传，服务端不支持时安全回退全量下载
- [x] 更新下载失败重试、取消和 `.part`/`.etag` 清理契约
- [x] 补充 GIF spool 失败取帧后的 discard 重试清理测试
- [x] 补充图片转换生命周期清理、短窗口下拉方向和焦点样式契约
- [x] 前端 208 项测试、Rust 140+3 项测试、fmt、Clippy 和 diff 检查通过

验收证据：`87284f2`、`7997ef4`、`6270148`、`e6631a7`；当前实现已推送到 `main`，待版本号与发布资产门禁完成后发布。
