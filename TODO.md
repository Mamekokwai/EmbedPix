...
验收：原生预检能阻止目录不存在、目录不可写和禁止覆盖的确定性失败，并给出逐项原因；覆盖策略按当前设置放行；浏览器环境显示未执行原生预检。磁盘空间探针当前 gateway 返回未检查（估算字节传 0），因此完整验收暂不勾选。现有 gateway 测试覆盖 native probe 调用和 unsupported browser，`npm test` 194 tests、`npm run build`、`git diff --check` 均通过。
