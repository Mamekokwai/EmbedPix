...
当前 E4 发布门禁审计：P0（E1/E2/E3）证据已归档；P1 的 E5/E6/E7 preview 编码契约已完成，E7 仍缺真实导出文件回读 harness，E8 的 ICC/色彩管理仍未完成，E9 预检/估算/Windows 磁盘空间闭环已完成但非 Windows 容量探针仍降级；P2 的 E10 仅完成 WebP/ICO/TIFF 评估，结论是不加入生产依赖或用户选项。版本/签名/manifest 门禁已有 `v0.3.2` 远端证据：Release 非 draft、7 个资产齐全、`latest.json` 两平台签名与 `.sig` 一致、provenance 指向发布 commit；这不能替代 `0.3.3` 的新版本一致性检查。发布前仍需归档 E8 证据、评估 E7 回读 harness 是否纳入承诺、准备包含兼容性/已知限制/回归数据的发布说明，并由用户决定 E10 不进入生产的范围；在此之前不勾选 E4 或创建 `0.3.3` Release。
...
验收：已覆盖默认清理策略、显式旋转不重复应用和透明像素回归；EXIF Orientation 读取、ICC profile 保留及色彩管理仍未完成，因此不勾选完整验收。
